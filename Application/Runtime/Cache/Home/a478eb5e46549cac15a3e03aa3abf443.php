        ...
        <div class="list_table_div mar_t20">
            <table class="list_table" cellspacing="1" cellpadding="0">
                ...
                <?php if(is_array($data["rows"])): $i = 0; $__LIST__ = array_slice($data["rows"],0,10,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rows): $mod = ($i % 2 );++$i;?><tr class="tr_line">
                        <td width="50"><input type="checkbox" class="id" name="id[]" value="<?php echo ($rows["article_type_id"]); ?>" /></td>
                        <td width="50"><?php echo ($rows["article_type_id"]); ?></td>
                        <td width="200"><a class="info" href="javascript:void(0);" title="<?php echo ($rows["article_type_name"]); ?>" value="<?php echo ($rows["article_type_id"]); ?>"><?php echo (substr_mb($rows["article_type_name"],0,10,'utf-8')); ?></a></td>
