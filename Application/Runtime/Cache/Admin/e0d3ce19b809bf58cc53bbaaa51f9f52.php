...
<div class="main_form">
    ...
    <div id="list_div">
        <div class="toolbar">
            <div class="lfloat">
                <input type="button" value="通过" id="add" />
                <input type="button" value="忽略" id="del" />
            </div>
            <div class="rfloat">
                <select id="keyItem">
                    ...
                    <option value="admin_id+like">审核员ID</option>
                </select>
            ...
            <table class="list_table" cellspacing="1" cellpadding="0">
                ...
                <?php if(is_array($data["rows"])): $i = 0; $__LIST__ = array_slice($data["rows"],0,10,true);if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$rows): $mod = ($i % 2 );++$i;?><tr class="tr_line">
                        ...
                        <?php if($rows["admin_name"] != null): ?><td><?php echo ($rows["admin_name"]); ?></td>
                            <?php else: ?>
                            <td>未审核</td><?php endif; ?>
                        <?php if($rows["pass_time"] != null): ?><td><?php echo ($rows["pass_time"]); ?></td>
                            <?php else: ?>
                            <td>未审核</td><?php endif; ?>
