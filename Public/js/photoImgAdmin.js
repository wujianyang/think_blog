$(document).ready(function(){
    ...
    $('#del').click(function(){
        ...
        if(arr.length==0){
            ...
            if(confirm('该相册的相关相片也会一起删除,确定删除？')){
                ...
                $.ajax({
                    url:"/think_blog/index.php/Admin/PhotoImg/del",
                    type:"post",
                    dataType:"json",
                    data:{id:arr},
    ...
    $('#add_form').validate({
        ...
        submitHandler:function(form){
            $(form).ajaxSubmit({
                url:"/think_blog/index.php/Admin/PhotoImg/add",
                type:"post",
                dataType:"json",
                success:function(data){
                    alert(data.msg);
    ...
    $('#edit_form').validate({
        ...
        submitHandler:function(form){
            $(form).ajaxSubmit({
                url:"/think_blog/index.php/Admin/PhotoImg/edit",
                type:"post",
                dataType:"json",
                success:function(data){
                    alert(data.msg);
    ...
    $('.info').live('click', function(){
        ...
        if(/\d/.test(id)){
            ...
            //根据id信息
            $.ajax({
                url:"/think_blog/index.php/Admin/PhotoImg/info",
                type:"post",
                data:{"id":id},
                dataType:"json",
...
function showList(page,page_size,keyItem,key,com){
    ...
    $.ajax({
        url:"/think_blog/index.php/Admin/PhotoImg/index",
        type:"post",
            ...
            if(data.status==1){
                ...
                if(rows.length>0) {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        sHtml+='<p><a class="info" href="javascript:void(0);" value="'+rows[i]['id']+'"><b>'+rows[i]['id']+'</b>：'+rows[i]['img_title']+'</a></p>';
                        sHtml+='<p><span>'+rows[i]['photo_id']+'</span>：'+rows[i]['photo_title']+'</p>';
                        sHtml+='<p><span>'+rows[i]['member_id']+'</span>：'+rows[i]['member_name']+'</p>';
                        sHtml+='</div>';
                    }
                    ...
                    $('#list_table_div').html('<div class="error">没有数据</div>');
                }
